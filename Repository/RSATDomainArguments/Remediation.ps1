...
if ($null -eq $PrimaryDNSSuffix) {
    Write-Output 'A primary DNS suffix is not set on the device however RSAT tools are installed. Exiting 1'
    Exit 1
}
...
if ($RSATShortcuts.Count -ge 1) {
    ...
    foreach ($RSATShortcut in $RSATShortcuts) {
        ...
        if (!$ShortcutDetails.Arguments.Contains("/domain=$PrimaryDNSSuffix")) {
            ...
            Write-Output "Updating the '$($RSATShortcut.name)' Shortcut to include the domain"

            Set-Shortcut -LinkPath $RSATShortcut.FullName -Arguments "/domain=$PrimaryDNSSuffix"
        
        }
        
    }

    Write-Output "RSAT tools updated"
    Exit 0
